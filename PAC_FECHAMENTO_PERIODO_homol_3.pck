...
CREATE OR REPLACE PACKAGE BODY "PAC_FECHAMENTO_PERIODO" IS
 ...
 -------------------------------------------------geral------------------------------------
  FOR GER IN  (
           SELECT DISTINCT  C1.DESC_MENSAGEM,H.COD_IDE_CLI,H.TIP_PROCESSO, H.NUM_GRP,H.COD_INS,H.PER_PROCESSO,C1.DAT_INI_VIG,C1.DAT_FIM_VIG -- INDIVIDUAL
  ...
  FOR GERI IN  (
           ...
           WHERE C1.COD_INS      =  H.COD_INS
            ...
            ORDER BY C1.NUM_PRIORIDADE
             ...
             AND   H.NUM_GRP       =  GERI.NUM_GRP;  
         END LOOP;

-------------------------------------------------Aniversario------------------------------------
    FOR ANI IN ( -- aniversário
         SELECT DISTINCT  C1.DESC_MENSAGEM,H.COD_INS,
         H.NUM_GRP,H.COD_IDE_CLI,H.PER_PROCESSO, H.TIP_PROCESSO,
...
---------------------------------------------INDIVIDUAL ----------------------------
  FOR IND IN  (
           SELECT DISTINCT  C1.DESC_MENSAGEM,H.COD_INS,H.TIP_PROCESSO,H.NUM_GRP,M.COD_IDE_CLI,H.PER_PROCESSO,C1.DAT_INI_VIG,C1.DAT_FIM_VIG -- INDIVIDUAL
           FROM TB_MENSAGEM_CTR C1,
                TB_HFOLHA_PAGAMENTO H,
