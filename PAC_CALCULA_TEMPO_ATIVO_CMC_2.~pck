...
SELECT COD_IDE_CLI,
 ...
 --
  ...
  BEGIN
   ...
   -- IF PAR_TIPO_CONTAGEM in (1, 2, 8, 9, 11) THEN  -- Roberto
 ...
 --    SP_CONTAGEM_CTC_INSS(SEQ_CERTIDAO);
  ...
  --  END IF;
    ...
    --- Bloque de Observações  Certidoes 1,2,3,9 ----
    FOR OBS_homol IN
      (
    ...
    LOOP
                        ...
                        FOR OBS_homol1 IN
                          (
                              SELECT 'Contagem suspensa no período entre 28/05/2020 e 31/12/2021, nos termos do inciso IX do art. 8º a LC 173/2020' AS OBS
                              ...
                              SELECT 'Contagem suspensa no período entre 28/05/2020 e 31/12/2021, nos termos do inciso IX do art. 8º a LC 173/2020' AS OBS
                               ...
                               WHERE T1.COD_IDE_CLI = T.COD_IDE_CLI
                                 ...
                                 (select 1 from tb_contagem_homologacao a where ID_CONTAGEM = SEQ_CERTIDAO
                                  and OBSERVACOES = 'Contagem suspensa no período entre 28/05/2020 e 31/12/2021, nos termos do inciso IX do art. 8º a LC 173/2020' )
                          )
                        LOOP
...
-- FIM
  ...
  BEGIN
    LEER_TEMPOS;
   -- IF (PAR_TIPO_CONTAGEM in ( 1,2,3,10,11,12,13) OR PAR_TIPO_CONTAGEM = 9) THEN -- Roberto
    ...
    ELSE
      IF PAR_TIPO_CONTAGEM = 2 THEN
        ...
        END LOOP;
      ELSE
        IF PAR_TIPO_CONTAGEM = 8 THEN -- ROBERTO
  ...
  BEGIN
   ...
   -- IF PAR_TIPO_CONTAGEM in (2,3) THEN Roberto
      IF PAR_TIPO_CONTAGEM in (8) THEN  
  ...
  --   IF PAR_TIPO_CONTAGEM in ( 1,2,8,10,11,12,13) OR PAR_TIPO_CONTAGEM = 9 THEN -- Roberto
      ---- Cursor de ATS
       IF PAR_TIPO_CONTAGEM  = 8 then
      OPEN CURTEMP_ATS;
  ...
  BEGIN
    ...
    --IF (PAR_TIPO_CONTAGEM IN (1,2,3,11) OR PAR_TIPO_CONTAGEM = 9 ) THEN -- Roberto
     IF PAR_TIPO_CONTAGEM = 8 then
  ...
  BEGIN
    ...
    VALUES
      (PAR_COD_INS,
       ...
       'CONTAGEM',
       CASE
         WHEN PAR_TIPO_CONTAGEM = 11 THEN 'CERTIDÃO DE TEMPO DE SERVIÇO PARA USOEM OUTRO ÓRGÃO'
         WHEN PAR_TIPO_CONTAGEM = 12 THEN 'CERTIDÃO DE AVERBAÇÃO DE TEMPO DE SERVIÇO DO RPPS PARA APOSENTADORIA'
         WHEN PAR_TIPO_CONTAGEM = 13 THEN 'CERTIDÃO DE AVERBAÇÃO DE TEMPO DE SERVIÇO'
         ELSE ''
       END);
...
/*    select
  ...
  where rf.cod_ins          = PAR_COD_INS
    and rf.cod_regime       = '2' --RGPS
    and rf.cod_plano        = '2' --RGPS
    and rf.tip_provimento   in ('2', '8') --Livre Provimento (cargo em Comissão)
...
select hct.dat_adm_orig     as data_inicio,
         ...
         SELECT CSO.DAT_INICIO,
                 ...
                 (SELECT UPPER(CO.DES_DESCRICAO)
                   ...
                   WHERE CO.COD_INS = 0
                     AND CO.COD_NUM = 2019
                     AND CO.COD_PAR = RF.COD_TIPO_DOC_ASSOC_FIM) || ' ' ||
                 RF.NUM_DOC_ASSOC_FIM || ', ' || 'COM PUBLICAÇÃO EM ' ||
 ...
 GROUP BY DAT_INICIO,
      ...
      FOR OBS_homol IN
        (
        SELECT
        'DECLARAMOS QUE A SERVIDORA FOI CONTRATADA PELO REGIME ESTATUTÁRIO. DECLARAMOS, AINDA, QUE AS CONTRIBUIÇÕES PREVIDENCIÁRIAS FORAM RECOLHIDAS PARA O REGIME GERAL DE PREVIDÊNCIA SOCIAL (INSS), CONFORME RESOLUÇÃO NO. 694/98 E EMENDA CONSTITUCIONAL NO. 20, DE 15/12/1998.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-'
        AS OBS
        FROM DUAL
        )
      LOOP
                        ...
                        FROM (
                              ...
                              SELECT 'INCLUSAO' as Tipo,
                                      ...
                                      CASE
                                        ...
                                        when COD_IDE_CLI = 1000032093 and NOM_CARGO = 'ASSISITENTE DE ADMINISTRAÇÃO' then 636
                                        ...
                                        WHERE RF.COD_INS = PAR_COD_INS
                                          ...
                                          AND ( (PAR_TIPO_CONTAGEM IN (1,9,11) AND MO.AUMENTA_CONT_ATS = 'S')
                                              OR
                                                (PAR_TIPO_CONTAGEM IN (2)   AND MO.AUMENTA_CONT_LICENCA= 'S')
                                              OR
                                                (PAR_TIPO_CONTAGEM IN (8)   AND FLG_DESC_APOSENTADORIA = 'S')
                                              )
                                          --- Contagem tipo 2
                              ...
                              select 'OUTRO' as Tipo,
                                ...
                                from (SELECT distinct t1.dat_adm_orig,
                                      ...
                                      AND RF.COD_VINCULO not in (/*'22', */'5', '6')
                                        ------------------------------------------------------
                                      AND PAR_TIPO_CONTAGEM NOT IN (8)
                                        ...
                                        /* 'em '|| RF.DAT_FIM_EXERC || ' - ' ||*/
                                           ...
                                           (SELECT CO.DES_DESCRICAO
                                             ...
                                             WHERE CO.COD_INS = 0
                                               ...
                                               AND CO.COD_PAR =
                                                   RF.COD_TIPO_DOC_ASSOC_FIM) ||
                                           ' nº ' || RF.NUM_DOC_ASSOC_FIM || '. ' ||
                                           'Publicação em ' || RF.Dat_Pub_Fim_Exerc || '. '
                                           )
                                          ,
                                          chr(10)) WITHIN GROUP(ORDER BY EF.DAT_INI_EFEITO)
...
/*
    ...
    IF (PAR_TIPO_CONTAGEM = 13) THEN
      FOR PER_ATS IN (SELECT PAR_ID_CONTAGEM as id_contagem,
                             ...
                             SUM(BRUTO) QTD_BRUTO,
                             CASE WHEN COD_IDE_CLI = 1012840 AND DATA_FIM = '10/10/2023' -- SOLICITAÇÃO PEDRO/RENATA PARA FICAR IGUAL A CERTIDÃO PMC 06/12/2023
                                        THEN sum( Inclusao) -1
