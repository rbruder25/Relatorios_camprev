...
---------------------------------------------------------------------------------------------
 ...
 -------------------------------------------------------------------------------------------        
  ...
  as
   ...
   begin  
     for resp in (
        select distinct  rp.cod_orgao,rp.cod_entidade,rf.num_matricula,rf.cod_ide_rel_func,rp.cod_ide_cli,pa.ide_codpar as login
                  from paride pa
                 join tb_pessoa_fisica pf     on pf.num_cpf = pa.ide_rut
                 join tb_orgao_responsavel rp on rp.cod_ide_cli = pf.cod_ide_cli
...
-- confere se login está cadastrado 
 ...
 IF v_cod_retorno = 'OK' then
    ...
    for registro in     (        
        ...
        and   l.cod_orgao = resp.cod_orgao
        and   (l.dat_fim is null or l.dat_fim >= sysdate)   
     group by 
                P.COD_IDE_CLI, 
                P.Nom_Pessoa_Fisica,
  ...
  begin  
     for resp in (
        select rp.cod_orgao,  rp.cod_entidade,rf.num_matricula,rf.cod_ide_rel_func,rp.cod_ide_cli,pa.ide_codpar as login
                  from paride pa
                 join tb_pessoa_fisica pf     on pf.num_cpf = pa.ide_rut
                 join tb_orgao_responsavel rp on rp.cod_ide_cli = pf.cod_ide_cli
                 join tb_relacao_funcional rf on rf.cod_ide_cli = pf.cod_ide_cli
...
-- confere se login está cadastrado 
 ...
 IF v_cod_retorno = 'OK' then
    ...
    select distinct
        ...
        and   rp.FLG_STATUS = 'V' or  rp.FLG_STATUS is null
        and   f.flg_assinatura_serv = 'S' and (f.flg_assinatura_resp <> 'S' or f.flg_assinatura_resp is null)
        and   rp.cod_orgao = resp.cod_orgao
        and   (l.dat_fim is null or l.dat_fim >= sysdate)   
     group by 
                P.COD_IDE_CLI, 
                P.Nom_Pessoa_Fisica,
